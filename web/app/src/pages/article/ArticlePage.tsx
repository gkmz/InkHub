  ...
  return <div className="article-page">
    ...
    <div className="article-layout">
      ...
      <aside className={`review-panel mobile-${tab}`}>
        {!isDraft && !hidePrimaryForHugoFlow && primary && PrimaryIcon && <div className="review-command-bar"><div><span className="eyebrow">下一步</span><strong>{primary.label}</strong><small>完成后继续处理文章发布流程</small></div><button className="primary" onClick={() => void primary.action()}><PrimaryIcon size={17} />{primary.label}</button>{notice && <span className="review-command-notice" role="status">{notice}</span>}</div>}
