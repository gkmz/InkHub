...
export function CreateTaxonomyTermDialog({ overview, onClose, onApplied, onCreated }: CreateTaxonomyTermDialogProps) {
  ...
  return <div className="dialog-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !busy) onClose(); }}>
    <section className="taxonomy-dialog" role="dialog" aria-modal="true" aria-labelledby="create-taxonomy-title">
      <header><div><p className="eyebrow">Hugo categories</p><h2 id="create-taxonomy-title">新建类目</h2></div><button type="button" aria-label="关闭" disabled={Boolean(busy)} onClick={onClose}><X size={18} /></button></header>
      <div className="taxonomy-form">
        <label htmlFor="taxonomy-name">类目名称</label>
        <input ref={nameInput} id="taxonomy-name" maxLength={160} value={name} onChange={(event) => { setName(event.target.value); setPreview(null); }} />
        <label htmlFor="taxonomy-description">类目说明</label>
