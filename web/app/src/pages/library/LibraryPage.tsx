...
export function LibraryPage({ onNavigate }: { onNavigate: (path: string) => void }) {
  ...
  return <div className="library-page">
    ...
    <div className="list-header selectable"><label className="select-all"><input ref={selectAll} type="checkbox" aria-label="选择当前已加载文章" aria-checked={someSelected && !allSelected ? "mixed" : allSelected} checked={allSelected} disabled={visibleItems.length === 0} onChange={() => setSelected(allSelected ? new Set() : new Set(visibleItems.map((article) => article.id)))} /></label><span>文章</span><span>修改时间</span><span>审核</span><span>Hugo</span><span>微信</span><span>小红书</span><span>操作</span></div>
