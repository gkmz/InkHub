...
export function DashboardPage({ onNavigate }: { onNavigate: (path: string) => void }) {
  ...
  return <div className="workspace-page"><section><div className="section-heading"><div><p className="eyebrow">按优先级</p><h2>需要处理</h2></div><span>{items.filter((item) => item.state !== "approved").length} 篇</span></div><div className="article-list">{items.filter((item) => item.state !== "approved").map((article) => <ArticleRow key={article.id} article={article} dashboard />)}</div></section></div>;
}
