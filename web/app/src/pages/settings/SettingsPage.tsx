...
export function SettingsPage() {
  ...
  return <div className="settings-page">
    ...
    <SettingsSection icon={<Globe2 />} title="发布渠道" description="Hugo 与微信公众号"><h3>微信公众号模板</h3><p className="fixed-template"><span>InkHub 墨绿</span><small>文章发布页可选择本次 Mermaid 样式</small></p><h3>GitHub 图片仓库</h3><label>GitHub Owner<input value={settings.github_owner ?? ""} onChange={(event) => setSettings({ ...settings, github_owner: event.target.value })} /></label><label>Repository<input value={settings.github_repository ?? ""} onChange={(event) => setSettings({ ...settings, github_repository: event.target.value })} /></label><div className="field-pair"><label>Branch<input value={settings.github_branch ?? "main"} onChange={(event) => setSettings({ ...settings, github_branch: event.target.value })} /></label><label>路径前缀<input value={settings.github_prefix ?? "inkhub"} onChange={(event) => setSettings({ ...settings, github_prefix: event.target.value })} /></label></div><SecretField label="GitHub Token" saved={settings.github_token_saved ?? settings.wechat_secret_saved} value={githubToken} onValueChange={setGitHubToken} onDelete={() => toast.show({ kind: "info", message: "请清空仓库配置后保存" })} /><button className="secondary" disabled={savingWeChat} onClick={() => void persistWeChat()}><Save size={15} />{savingWeChat ? "正在保存…" : "保存发布设置"}</button></SettingsSection>
