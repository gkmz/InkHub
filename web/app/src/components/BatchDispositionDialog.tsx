...
interface BatchDispositionDialogProps {
  ...
  count: number;
  channels: Partial<Record<PublicationChannel, boolean>>;
  busy?: boolean;
  onClose: () => void;
...
const channelLabels: Record<"hugo" | "wechat", string> = { hugo: "Hugo", wechat: "微信" };
...
export function BatchDispositionDialog({ mode, count, channels, busy = false, onClose, onConfirm, onOpenSettings }: BatchDispositionDialogProps) {
  ...
  const dispositionChannels = ["hugo", "wechat"] as const;
  const selectedChannels = dispositionChannels.filter((channel) => selected.has(channel)) as PublicationChannel[];
