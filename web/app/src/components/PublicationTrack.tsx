...
export function PublicationTrack({ review, hugo, wechat }: { review: string; hugo: string; wechat: string }) {
  return <section className="publication-track" aria-label="发布进度">{[["审核", review], ["Hugo", hugo], ["微信", wechat]].map(([label, state], index) => <div key={label} className={state.includes("失败") ? "failed" : state.includes("已") ? "complete" : "pending"}>{index > 0 && <i />}<span>{state.includes("失败") ? <CircleAlert /> : state.includes("已") ? <Check /> : <Circle />}</span><b>{label}</b><small>{state}</small></div>)}</section>;
}
