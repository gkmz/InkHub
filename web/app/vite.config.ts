...
const demoAPI: Plugin = {
  ...
  configureServer(server) {
    server.middlewares.use("/api/v1", async (request, response) => {
      ...
      else if (url.pathname === "/hugo-previews/preview-restored/render/posts/restored-writing-system/") {
        response.setHeader("Content-Type", "text/html; charset=utf-8");
        response.setHeader("Content-Security-Policy", "default-src 'none'; style-src 'unsafe-inline'; img-src data:;");
        response.end("<!doctype html><html lang=\"zh-CN\"><head><meta charset=\"utf-8\"><title>从笔记到发布</title><style>body{font-family:system-ui,sans-serif;max-width:720px;margin:48px auto;padding:0 24px;line-height:1.8;color:#20332b}h1{font-size:32px}h2{margin-top:32px;color:#16704b}blockquote{border-left:3px solid #8ac7a8;padding-left:16px;color:#557064}</style></head><body><p>Hugo staging 渲染结果</p><h1>从笔记到发布：我的写作系统</h1><p>这是一份来自当前 staging Artifact 的真实页面预览。</p><h2>先整理，再发布</h2><p>InkHub 会把同一篇 Obsidian 原文转换为适合博客展示的页面。</p><blockquote>确认后才会写入 Hugo 博客目录。</blockquote></body></html>");
        return;
      }
