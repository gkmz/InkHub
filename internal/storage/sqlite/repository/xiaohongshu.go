...
id,article_id,workspace_id,source_content_hash,title,body_html,topics_json,source_note,comment_copy,ai_model,prompt_version,state,created_at,updated_at)
VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)
ON CONFLICT(id) DO UPDATE SET title=excluded.title,body_html=excluded.body_html,topics_json=excluded.topics_json,
...
WHERE xiaohongshu_drafts.article_id=excluded.article_id AND xiaohongshu_drafts.workspace_id=excluded.workspace_id`,
		draft.ID, draft.ArticleID, draft.WorkspaceID, draft.SourceContentHash, draft.Title, draft.BodyHTML, string(topics), draft.SourceNote, draft.CommentCopy, draft.AIModel, draft.PromptVersion, draft.State, now, now)
	if err != nil {
...
func (r *XiaohongshuRepository) FindDraft(ctx context.Context, workspaceID, articleID, id string) (domain.Draft, error) {
	return scanDraft(r.db.QueryRowContext(ctx, `SELECT id,article_id,workspace_id,source_content_hash,title,body_html,topics_json,source_note,comment_copy,ai_model,prompt_version,state,created_at,updated_at FROM xiaohongshu_drafts WHERE workspace_id=? AND article_id=? AND id=?`, workspaceID, articleID, id))
}
...
func (r *XiaohongshuRepository) ListDrafts(ctx context.Context, workspaceID, articleID string, limit int) ([]domain.Draft, error) {
	...
	rows, err := r.db.QueryContext(ctx, `SELECT id,article_id,workspace_id,source_content_hash,title,body_html,topics_json,source_note,comment_copy,ai_model,prompt_version,state,created_at,updated_at FROM xiaohongshu_drafts WHERE workspace_id=? AND article_id=? ORDER BY created_at DESC,id DESC LIMIT ?`, workspaceID, articleID, limit)
	if err != nil {
...
func scanDraft(row rowScanner) (domain.Draft, error) {
	...
	if err := row.Scan(&item.ID, &item.ArticleID, &item.WorkspaceID, &item.SourceContentHash, &item.Title, &item.BodyHTML, &topicsJSON, &item.SourceNote, &item.CommentCopy, &item.AIModel, &item.PromptVersion, &item.State, &item.CreatedAt, &item.UpdatedAt); err != nil {
