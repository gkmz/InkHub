...
func (r *TemplateRepository) Activate(ctx context.Context, id, version, digest, path string) error {
	if r.workspaceID == "" || id == "" || version == "" || digest == "" || path == "" {
	...
	defer tx.Rollback()
	if _, err := tx.ExecContext(ctx, `UPDATE templates SET enabled=0,updated_at=? WHERE workspace_id=? AND enabled=1`, nowText(), r.workspaceID); err != nil {
...
id,workspace_id,template_id,version,source,manifest_json,install_path,enabled,created_at,updated_at)
VALUES (?,?,?,?,?,?,?,?,?,?)
...
WHERE templates.manifest_json=excluded.manifest_json AND templates.install_path=excluded.install_path`,
		stableID, r.workspaceID, id, version, "local", string(manifest), path, 1, now, now)
	if err != nil {
