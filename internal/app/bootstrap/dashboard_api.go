...
WHERE articles.deleted_at IS NULL AND articles.content_stage='ready' AND articles.workspace_id=` + currentWorkspaceSQL + `
...
func (api databaseAPI) Dashboard(ctx context.Context) (httptransport.DashboardView, error) {
	view := httptransport.DashboardView{
		...
		ReadyToPublish:  []httptransport.ArticleSummary{},
		LatestReady:     []httptransport.ArticleSummary{},
	...
	for rows.Next() {
		row, scanErr := scanDashboardArticle(rows, channels)
		if scanErr != nil {
			return view, scanErr
		}
		...
		case row.ignored():
			continue
		case row.bucket == "failed":
			view.Failed = append(view.Failed, row.summary)
		case row.bucket == "changed":
			view.Changed = append(view.Changed, row.summary)
		case row.bucket == "needs_review":
			view.NeedsReview = append(view.NeedsReview, row.summary)
		case row.bucket == "ready_to_publish":
			view.ReadyToPublish = append(view.ReadyToPublish, row.summary)
		case row.bucket == "latest_ready":
			if len(view.LatestReady) < 10 {
				view.LatestReady = append(view.LatestReady, row.summary)
			}
		case row.bucket == "recently_handled":
...
func scanDashboardArticle(rows interface{ Scan(...any) error }, channels []string) (dashboardArticleRow, error) {
