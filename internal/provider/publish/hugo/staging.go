...
func (p *Provider) Prepare(ctx context.Context, input contracts.PublishInput) (contracts.PreparedArtifact, error) {
	...
	if discovery.SelectionLocked {
		...
		desiredTarget := filepath.Join(contentRoot, section, filepath.FromSlash(directory), bundleSegment(input))
		...
		target = filepath.Join(contentRoot, section, filepath.FromSlash(directory), segment)
		change = "added"
